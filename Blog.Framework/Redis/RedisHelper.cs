...
namespace Infrastructure.Common.Redis
{
    ...
    public class RedisHelper
    {
        ...
        private static RedisHelper _redis { get; set; }
        public static RedisHelper Redis
        {
            get
            {
                if (_redis == null) _redis = new RedisHelper();
                return _redis;
            }
        }
        #region 构造函数
        ...
        public RedisHelper(int dbNum, string readWriteHosts)
        {
            ...
            {
                LogHelper.WriteTextLog("Redis", ex);
            }
        }
        ...
        /// <param name="expiry">过期时间</param>
        /// <returns></returns>
        public bool SetDefault<T>(string key, T obj, TimeSpan? expiry = default(TimeSpan?))
        {
            key = AddSysCustomKey(key);
            string json = ConvertJson(obj);
            if (expiry == null)
                expiry = new TimeSpan(5, 0, 0);
        ...
        public T StringGet<T>(string key)
        {
            if (_conn != null)
            {
                ...
                return Do(db =>
                {
                    var info = db.StringGet(key);
                    if (info.IsNull)
                        return default(T);

                    var keyValueString = Encoding.UTF8.GetString(info);
                    if (typeof(T) == typeof(String) && !keyValueString.StartsWith("\""))
                    {
                        keyValueString = "\"" + keyValueString + "\"";
                    }
                    return ConvertObj<T>(keyValueString);
                });
            }
            else
            {
        ...
        public T ListGetByIndex<T>(string key, long index)
        {
            ...
            return Do(redis =>
            {
                if (redis.ListLength(key) > index)
                {
                    var values = redis.ListGetByIndex(key, index);
        ...
        /// <param name="key"></param>
        /// <param name="value"></param>
        public void ListInsertAfter<T>(string key, T pervalue, T value)
        {
            key = AddSysCustomKey(key);
            Do(db => db.ListInsertAfter(key, ConvertJson(pervalue), ConvertJson(value)));
        }
        ...
        /// <returns></returns>
        public List<string> GetPatternKey(string pattern)
        {
            var database = _conn.GetDatabase(DbNum);
            var server = GetServer(Config.ConfigKeys.GetGeneralConfiguration("TY00001", "Domain_Redis"));
            var keys = server.Keys(pattern: "*" + pattern + "*", database: database.Database);
