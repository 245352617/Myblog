...
namespace Infrastructure.Common.XML
{
    ...
    {
        private static ILogger<XmlHelper> Logger;

        public XmlHelper(ILogger<XmlHelper> _logger)
        {
            Logger = _logger;
        }
        ...
        {
            var xmlWriterSettings = new XmlWriterSettings { OmitXmlDeclaration = omitXmlDeclaration,NewLineOnAttributes=true,NewLineChars="\r\n" };
        ...
        /// 根据类型自动配置文件名
        /// </summary>
        /// <typeparam name="fileName">文件名，缺省取类名</typeparam>
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public static string GetDefaultFilePath<T>(string fileName=null)
        {
            if (string.IsNullOrWhiteSpace(fileName)) fileName = (typeof(T).Name);
        ...
        /// <param name="filePath">文件路径，为null则自动以类型名保存到执行目录</param>
        /// <param name="config"></param>
        public static void XmlSerialize<T>(T tObj, string filePath) where T : class, new()
        {
            if(string.IsNullOrEmpty(filePath))filePath= GetDefaultFilePath<T>();
            var fileMode = File.Exists(filePath) ? FileMode.Truncate : FileMode.OpenOrCreate;
            using (var fs = new FileStream(filePath, fileMode))
            {
                var xs = new XmlSerializer(typeof(T));
                xs.Serialize(fs, tObj);
                fs.Close();
            }
        }
        ...
        /// Xml反序列化加载对象
        /// </summary>
        /// <param name="defaultTObj">初始对象,第一次使用自动保存</param>
        /// <param name="filePath">文件路径，为null则自动以类型名保存到执行目录</param>
        /// <returns></returns>
        public static T XmlDeserialize<T>(T defaultTObj = default(T),string filePath = null) where T : class, new()
        {
            if (string.IsNullOrEmpty(filePath)) filePath = GetDefaultFilePath<T>();
            if (!File.Exists(filePath))
            {
                if (defaultTObj != null)//初始化对象保存
                {
                    XmlSerialize(defaultTObj, filePath);
                }
                return defaultTObj;
            }
            return XmlFileDeserialize<T>(filePath);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static T XmlDeserialize<T>() where T : class, new()
        {
            return XmlDeserialize<T>(defaultTObj:default(T), filePath:null);
        }
